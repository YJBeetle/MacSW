...
NOTO_SANS_SC_DIR="${CONTENTS_DIR}/Resources/fonts/NotoSansSC"
...
test "$(/usr/libexec/PlistBuddy -c 'Print :MacSWNotoSansSCVersion' "${INFO_PLIST}")" = "${NOTO_SANS_SC_VERSION}"
test "$(/usr/libexec/PlistBuddy -c 'Print :MacSWNotoSansSCRegularSHA256' "${INFO_PLIST}")" = "${NOTO_SANS_SC_REGULAR_SHA256}"
test "$(/usr/libexec/PlistBuddy -c 'Print :MacSWNotoSansSCBoldSHA256' "${INFO_PLIST}")" = "${NOTO_SANS_SC_BOLD_SHA256}"
...
test "$(shasum -a 256 "${NOTO_SANS_SC_REGULAR}" | awk '{print $1}')" = "${NOTO_SANS_SC_REGULAR_SHA256}"
test "$(shasum -a 256 "${NOTO_SANS_SC_BOLD}" | awk '{print $1}')" = "${NOTO_SANS_SC_BOLD_SHA256}"
test "$(shasum -a 256 "${NOTO_SANS_SC_LICENSE}" | awk '{print $1}')" = "${NOTO_SANS_SC_LICENSE_SHA256}"
...
test "$(/usr/libexec/PlistBuddy -c 'Print :NotoSansSCVersion' "${BUILD_MANIFEST}")" = "${NOTO_SANS_SC_VERSION}"
test "$(/usr/libexec/PlistBuddy -c 'Print :NotoSansSCArchiveSHA256' "${BUILD_MANIFEST}")" = "${NOTO_SANS_SC_ARCHIVE_SHA256}"
test "$(/usr/libexec/PlistBuddy -c 'Print :NotoSansSCRegularSHA256' "${BUILD_MANIFEST}")" = "${NOTO_SANS_SC_REGULAR_SHA256}"
test "$(/usr/libexec/PlistBuddy -c 'Print :NotoSansSCBoldSHA256' "${BUILD_MANIFEST}")" = "${NOTO_SANS_SC_BOLD_SHA256}"
test "$(/usr/libexec/PlistBuddy -c 'Print :NotoSansSCLicenseSHA256' "${BUILD_MANIFEST}")" = "${NOTO_SANS_SC_LICENSE_SHA256}"
